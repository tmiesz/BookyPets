...
public class GetBooksQueryHandler(IBooksRepository booksRepository) : IHandler<GetBooksQuery, Result<List<Book>>>
{
    private readonly IBooksRepository _booksRepository = booksRepository;

    public async Task<Result<List<Book>>> HandleAsync(GetBooksQuery request, CancellationToken cancellationToken = default)
    {
